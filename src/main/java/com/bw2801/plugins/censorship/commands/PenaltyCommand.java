    ...
    public boolean onCommand(CommandSender cs, Command cmnd, String string, String[] args) {
        ...
        if (args.length == 4 && args[1].equalsIgnoreCase("add")) {
            ...
            try {
                ...
                cs.sendMessage(ChatColor.WHITE + "Successfully added \"" + ChatColor.RED + args[3] + ChatColor.WHITE + "\" penalty points for \"" + ChatColor.GOLD + args[2] + ChatColor.WHITE + "\".");
            } catch(NumberFormatException ex) {
                cs.sendMessage(ChatColor.RED + "Could not add \"" + args[3] + "\" penalty points for \"" + args[2] + "\".");
            }
            ...
            try {
                ...
                cs.sendMessage(ChatColor.WHITE + "Successfully removed \"" + ChatColor.RED + args[3] + ChatColor.WHITE + "\" penalty points from \"" + ChatColor.GOLD + args[2] + ChatColor.WHITE + "\".");
            } catch(NumberFormatException ex) {
                cs.sendMessage(ChatColor.RED + "Could not remove \"" + args[3] + "\" penalty points from \"" + args[2] + "\".");
            }
        } else if (args.length == 3 && args[1].equalsIgnoreCase("get")) {
            if (!Censorship.hasPermission(cs, "censor.penalty-points.get")) return true;
            cs.sendMessage(ChatColor.GOLD + args[2] + ChatColor.WHITE + " has " + ChatColor.RED + PlayerHandler.getPenaltyPoints(args[2]) + ChatColor.WHITE + " penalty points.");
        } else {
            return false;
        }
